 ...
 */
function hc_msg91_send_transactional_sms($mobile, $flow_id, $vars = [], $force_send = false) {
...
function hc_msg91_sms_on_new_customer_registration($user_id) {
    ...
    if (!get_option('msg91_sms_ncr_enable', 0)) return;
...
function hc_msg91_sms_on_new_order_placed($order_id, $posted_data, $order) { // Expects 3 args
...
function hc_msg91_sms_on_thankyou_page($order_id) { // Expects 1 arg
...
function hc_msg91_schedule_abandoned_cart_check() {
    ...
    if (is_admin() || !get_option('msg91_sms_oac_enable', 0)) return;
    ...
    $user_id = get_current_user_id();
    if (!$user_id) return; // Only for logged-in users for simplicity
    ...
    if ($delay_hours <= 0) $delay_hours = 1;
    ...
    wp_clear_scheduled_hook('hc_msg91_trigger_abandoned_cart_sms', [$user_id, $cart_hash]); // Old hash might be different
    ...
    if (!wp_next_scheduled('hc_msg91_trigger_abandoned_cart_sms', [$user_id, $cart_hash])) {
        wp_schedule_single_event(time() + ($delay_hours * HOUR_IN_SECONDS), 'hc_msg91_trigger_abandoned_cart_sms', [$user_id, $cart_hash]);
    }
}
...
function hc_msg91_send_abandoned_cart_sms($user_id, $scheduled_cart_hash) {
    ...
    if (WC()->session && is_callable([WC()->session, 'get_customer_id']) && WC()->session->get_customer_id() != $user_id) {
    ...
    if (!empty($orders)) return; // User placed an order
...
function hc_msg91_clear_abandoned_cart_check_on_order($order_id) {
    ...
    if ($order && $order->get_customer_id()) {
        ...
        $timestamp = wp_next_scheduled('hc_msg91_trigger_abandoned_cart_sms', [$user_id, null]); // This won't work directly
...
function hc_msg91_add_shipment_details_meta_box() {
    add_meta_box(
        ...
        'shop_order', // Post type for WooCommerce orders
...
function hc_msg91_shipment_details_meta_box_html($post) {
    // Use $post->ID to get the order ID
    $order_id = $post->ID;
    $tracking_id = get_post_meta($order_id, '_hc_msg91_tracking_id', true);
    $tracking_url = get_post_meta($order_id, '_hc_msg91_tracking_url', true);
    $shipping_provider = get_post_meta($order_id, '_hc_msg91_shipping_provider', true);
    ...
    ?>
    <p>
        <label for="hc_msg91_tracking_id"><?php _e('Tracking ID:', 'hc-msg91-otp'); ?></label><br>
        <input type="text" id="hc_msg91_tracking_id" name="hc_msg91_tracking_id" value="<?php echo esc_attr($tracking_id); ?>" style="width:100%;">
    </p>
    <p>
        <label for="hc_msg91_tracking_url"><?php _e('Tracking URL (optional if provider known):', 'hc-msg91-otp'); ?></label><br>
        <input type="url" id="hc_msg91_tracking_url" name="hc_msg91_tracking_url" value="<?php echo esc_attr($tracking_url); ?>" style="width:100%;">
    </p>
    <p>
        <label for="hc_msg91_shipping_provider"><?php _e('Shipping Provider (optional):', 'hc-msg91-otp'); ?></label><br>
    ...
    <p class="description">
        <?php _e('Enter the full tracking URL, or just the ID if you intend to construct the URL in your SMS template variables (e.g., someprovider.com/track={TRACKING_ID}).', 'hc-msg91-otp'); ?>
    </p>
    <?php
}

// Save the custom meta fields
add_action('save_post_shop_order', 'hc_msg91_save_shipment_details_meta', 10, 1);
